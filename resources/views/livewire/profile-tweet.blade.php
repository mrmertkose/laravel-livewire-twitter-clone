<div>
    @if($user->tweets->count())
        @foreach($user->tweets as $item)
            <div class="p-3 border-bottom border-custom"
                 wire:key="{{$item->id}}">
                <div class="row">
                    <div class="col-lg-2 d-flex align-items-center">
                        <a href="{{route("profile",$item->user->username)}}">
                            <img class="rounded-circle img-fluid" src="{{asset("images/user/".$item->user->image)}}"
                                 alt="{{$item->user->name}}"
                                 width="75">
                        </a>
                    </div>
                    <div class="col-lg-10 pb-2">
                        <span class="font-weight-bold"><a
                                href="{{route("profile",$item->user->username)}}"> {{$item->user->name}}</a></span>
                        <span class="text-muted"><a
                                href="{{route("profile",$item->user->username)}}">• {{$item->user->username}}</a> </span>
                    ...
                    <div class="offset-2 col-lg-10 d-flex justify-content-start">
                        ...
                        @if($item->favorite()->data->count())
                            <span><a wire:click="unFavorites({{$item->id}})"><i
                        ...
                        @else
                            <span><a wire:click="favorites({{$item->id}})"><i
